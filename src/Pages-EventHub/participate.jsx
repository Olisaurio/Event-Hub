...
const Participate = () => {
  ...
  // Función para formatear la fecha de inscripción
  const formatInscriptionDate = (date) => {
    const inscriptionDate = new Date(date);
    return inscriptionDate.toLocaleDateString('es-ES', {
      day: 'numeric',
      month: 'short',
      year: 'numeric'
    });
      ...
      {inscriptions && inscriptions.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {inscriptions.map((inscription) => {
            const event = inscription.evento;
            return (
              <Link 
                key={inscription.id} 
                to={`/event/${event.id}`}
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-gray-600 text-sm">
                        Inscrito: {formatInscriptionDate(inscription.fechaInscripcion)}
                      </span>
                    </div>
