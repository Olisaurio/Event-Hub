  ...
  // --- Componente Uploader Galería (Solo Imágenes - Estructura Revisada) ---
  const GalleryImageUploader = ({ images, previews, onImagesUpload, onImageRemove, error }) => {
      const fileInputRef = useRef(null);
      const handleButtonClick = () => fileInputRef.current?.click();
      return (
          <div className={`gallery-uploader form-group ${error ? 'error' : ''}`}> {/* Added form-group */}
              ...
              <div className="gallery-previews-container">
                  {previews.map((preview, index) => (
                      <div key={index} className="gallery-preview-item">
                          ...
                          <img src={preview} alt={`Galería ${index + 1}`} className="gallery-image-preview" /> {/* Changed class for clarity */}
                          {/* Botón de eliminar */}
                          <button
                              type="button"
                              className="remove-gallery-image-btn" // Changed class for clarity
              ...
              {/* Botón para añadir más imágenes (fuera de la cuadrícula) */}
              <button type="button" className="add-gallery-image-btn button-secondary" onClick={handleButtonClick}> {/* Changed to secondary style */}
                  <PlusIcon />
  ...
  const CreateEvent = () => {
    ...
    const processMediaFile = (file) => {
        return new Promise((resolve, reject) => {
            ...
            const maxSize = 50 * 1024 * 1024; // 50MB limit for both image and video
